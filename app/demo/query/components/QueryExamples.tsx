  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-2">
        {users?.voList.map((user) => (
          <div
            key={user.tenantId}
            className="p-3 border rounded-lg hover:bg-accent/50 transition-colors"
          >
            <div className="font-medium">{user.companyName}</div>
            <div className="text-sm text-muted-foreground">{user.domain}</div>
          </div>
        ))}
      </div>
      ...
      >
        <SvgIcon
          animate={isLoading ? "spin" : "click-pulse"}
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold">Mock 数据演示</h3>
        <Button
          ...
          {mockQuery.isLoading ? (
            <div className="flex items-center gap-2">
              <SvgIcon
                ...
                className="animate-spin"
              />
              <span>加载 Mock 数据...</span>
            </div>
          ) : mockQuery.error ? (
            <div className="text-red-600">
              <p className="font-semibold">请求失败</p>
              <p className="text-sm">{mockQuery.error.message}</p>
            ...
            <div className="space-y-2">
              <p className="font-medium">{mockQuery.data.message}</p>
              <p className="text-sm text-muted-foreground">
                时间戳: {mockQuery.data.timestamp}
              </p>
              <div className="text-xs text-muted-foreground">
                数据项数: {mockQuery.data.data?.length || 0}
              </div>
            </div>
          ) : null}
        </div>
...
export function TanStackQueryExamples() {
  ...
  const tabs = [
    ...
    { id: "mock", label: "Mock演示", component: MockDataExample },
  ];
